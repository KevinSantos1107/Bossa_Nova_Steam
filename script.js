...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ── 11. THEME TOGGLE ──
     O CSS universal já aplica transition nas cores de TODOS os elementos.
     Aqui só precisamos trocar o data-theme — o navegador cuida do resto. */
  (function initThemeToggle() {
    const btn = document.getElementById('themeToggle');
    if (!btn) return;
    const root = document.documentElement;

    // Remove qualquer overlay residual de versões anteriores
    const oldOverlay = document.getElementById('theme-overlay');
    ...
    // Set inicial — aplica sem transição para não animar no carregamento da página
    root.style.setProperty('--theme-transition', 'none');
    applyTheme(root.getAttribute('data-theme') || 'light');
    // Reativa a transição após o primeiro frame
    requestAnimationFrame(() => root.style.removeProperty('--theme-transition'));
  })();
